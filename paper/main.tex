...
\title{\Large \bf \texorpdfstring{A Conditional Spin\(^c\) Dolbeault Derivation of Three Standard Model Family Copies in PSLT}{A Conditional Spinc Dolbeault Derivation of Three Standard Model Family Copies in PSLT}}
...
h^1(\CP^1,\cO(2))=0.
\]
Provided all other SM-charged internal sectors are absent from the kernel or fully paired and gapped, the four-dimensional low-energy chiral spectrum contains exactly three unpaired SM family copies.
...
Related cohomological or topological mechanisms for chiral matter appear in heterotic, brane, and F-theory constructions \cite{Candelas1985,Witten1986,Blumenhagen2007,BeasleyHeckmanVafa2009}.
...
\node[right] at (-4.95,0.85) {\(z_{\rm ax}=+D/2\)};
\node[right] at (-4.95,-1.45) {\(z_{\rm ax}=-D/2\)};
...
\node[psltthin, text width=0.34\linewidth] at (-3.25,-2.45)
...
\fill (3.1,-1.3) circle (2pt) node[below] {\(p_-=[0:1]\)};
\node[psltoutput, text width=0.30\linewidth] at (3.1,-2.65)
{\(R=p_+ + p_-\)\\
\(\deg R=2\)\\
\(L_R=\cO(R)\simeq\cO(2)\)};
\end{tikzpicture}
...
\node[psltinput, text width=0.18\linewidth, below=of fixed, xshift=-3.5cm] (omega) {\(\Omega(\rho,z;D)\)};
\node[psltbox, text width=0.18\linewidth, right=of omega] (veff) {\(V_{\rm eff}\)};
\node[psltbox, text width=0.22\linewidth, right=of veff] (scalar_modes) {\(L_\Omega\psi_n=\omega_n^2\psi_n\)};
\node[psltbox, text width=0.22\linewidth, right=of scalar_modes] (locking) {\(\mu_{nN}\)\\hard/soft assignment};
\node[psltbox, text width=0.22\linewidth, below=0.8cm of scalar_modes] (rates2) {\(\Gamma_N^{\rm eff},\ B_N\)};
\node[psltoutput, text width=0.30\linewidth, right=of rates2] (prob) {\(\displaystyle P_N(t)=
...
\draw[psltarrow] (scalar_modes) -- (locking);
\draw[psltarrow] (locking.south) |- (rates2.east);
\draw[psltarrow] (rates2) -- (prob);
