...
\[
P_N(t)=
\frac{B_Ng_N^{\rm chiral}(1-\ee^{-\Gamma_Nt})}
{\sum_{K=1}^{3}B_Kg_K^{\rm chiral}(1-\ee^{-\Gamma_Kt})}.
\]
Thus the generation count is a Spin\(^c\) index-theoretic consequence of the two-center projection divisor, while \((g_N^{\rm chiral},\Gamma_N,B_N)\) describe degeneracy, kinetics, and visible-sector overlap of the three protected modes.
\end{abstract}
...
To attach formation data to the protected generations, the effective PSLT layer model includes a mode-locking prescription
\begin{equation}
\boxed{
\mathcal I_D:
\cH_{\rm gen}
...
H^0(C,L_R)
\longrightarrow
\cH_{\Omega}^{\rm scalar}(D),
\qquad
\mathcal I_D(u_N)
=
\psi_{n(N;D)}(\cdot;D).
}
\label{eq:mode_locking_map}
\end{equation}
Equivalently,
\begin{equation}
\psi_N^{\Omega}(\cdot;D)
...
:=
\omega_{n(N;D)}(D).
\label{eq:locked_scalar_mode}
\end{equation}
The map \(\mathcal I_D\) is not part of the Spin\(^c\) index theorem.
It is an effective interface datum that assigns scalar barrier and overlap data to the three protected generation modes after the generation count has already been fixed.
Thus
\begin{equation}
u_N
\in H^0(C,L_R)
\quad\text{fixes the chiral generation label,}
\qquad
\psi_N^\Omega
\quad\text{supplies scalar formation data.}
\label{eq:uN_psiN_distinction}
\end{equation}
All quantities \(\omega_N\), \(\Gamma_N\), and \(|N,\pm\rangle\) below are understood after this mode-locking prescription.

For kinetics, the locked scalar mode \(\psi_N^\Omega\) and the two centers define a collective-coordinate subspace
\begin{equation}
...
For visibility, use the projection map \(\pi_D\) defined in Eq.~\eqref{eq:piD_def} and let
\begin{equation}
\widehat u_N(\rho,z_{\rm ax},\varphi;D,\varepsilon)
=
u_N(\pi_D(\rho,z_{\rm ax},\varphi)).
\label{eq:uN_pullback}
\end{equation}
The \(\varphi\)-dependence is therefore not discarded; in axially reduced formulas it has already been projected into the selected angular channel and absorbed into the frame/Jacobian factor \(W_{\rm frame}\).
...
\overline{\widehat u_N(\rho,z_{\rm ax},\varphi;D,\varepsilon)}
\\
&\times
f_L(\rho,z_{\rm ax})
f_R(\rho,z_{\rm ax})
...
\label{eq:y_eff}
\end{equation}
When the visible kernel has already been reduced to an \(m=0\) axial channel, Eq.~\eqref{eq:y_eff} reduces to the earlier \(2\pi\rho\,\dd\rho\,\dd z_{\rm ax}\) form with the angular integral included in \(W_{\rm frame}\).
...
After the explicit mode-locking map \(\mathcal I_D:u_N\mapsto\psi_N^\Omega\), the scalar two-center spectral operator and the quantities \((g_N^{\rm chiral},\Gamma_N,B_N,P_N)\) define an effective model for how these three protected modes form and couple to the visible sector.
