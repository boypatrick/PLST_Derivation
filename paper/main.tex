...
For visibility, use the projection map \(\pi_D\) defined in Eq.~\eqref{eq:piD_def}.
Since \(u_N\) is a section of \(L_R\), its spatial representative is not a scalar function in an invariant formulation.
It is the pullback section
\begin{equation}
...
\end{equation}
where \(h_R\) is the Hermitian metric on \(L_R\).
