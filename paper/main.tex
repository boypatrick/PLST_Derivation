...
\begin{abstract}
We formulate the Projection Spectral Layer Theory (PSLT) as a first-principles projection-spectral derivation of the three-generation Standard Model.
...
h^1(\CP^1,\cO(2))=0.
\]
Provided the SM-charged chiral carrier is exactly this Spin\(^c\) generation sector and all other SM-charged internal sectors are vectorlike or gapped, the low-energy four-dimensional chiral spectrum contains exactly three Standard Model families.
The same two-center geometry also produces the conformal factor \(\Omega\) from a variational Poisson constraint, the scalar spectral operator \([-\nabla^2+V_{\rm eff}]\psi_N=\omega_N^2\psi_N\), and the formation/visibility probability law
\[
P_N(t)=
...
\end{equation}
All other SM-charged internal sectors are required to be vectorlike or gapped:
\begin{equation}
...
\cE_{\rm vec},
\qquad
\operatorname{ind}\scrD_{\rm vec}=0.
...
Assume all other SM-charged internal sectors are vectorlike or gapped, and all remaining chiral sectors are SM-neutral.
...
\dim\Ker\scrD_R^-=0.
\end{equation}
The exclusivity condition in Eq.~\eqref{eq:exclusivity_vec} prevents any additional SM-charged nonzero-index sector from contributing another chiral family.
\end{proof}
...
The SM-charged internal fermions are governed by the Spin\(^c\) Dolbeault--Dirac operator on \(\Lambda^{0,*}T^*C\otimes L_R\), so their positive and negative internal zero modes are \(H^0(C,L_R)\) and \(H^1(C,L_R)\).
Riemann--Roch gives index three, and Serre duality gives \(H^1(C,L_R)=0\).
With no additional SM-charged nonzero-index sector, the theory therefore has exactly three low-energy chiral SM generations.
