...
\end{equation}
To attach scalar barrier data to a protected generation, introduce a scalarization operator
\begin{equation}
\boxed{
\mathcal C_D:\cH_{\rm gen}\longrightarrow \cH_\Omega(D),
\qquad
\mathcal C_Du
=
\mathcal N_D(u)^{-1}
...
\end{equation}
Here \(\mathfrak j_D\in\Gamma(\mathbb R^3,\widehat L_R)\) is the locking/probe section supplied by the two-center projection data, \(w_D\ge0\) is a scalar density weight, and \(\mathcal N_D(u)\) normalizes the profile in \(\cH_\Omega(D)\).
The scalarization is assumed nonzero on the protected modes \(u_N\).
...
Thus the Spin\(^c\) theorem fixes \(u_N\), while \(\mathcal C_D\), \(\mu_{nN}\), and, only under Eq.~\eqref{eq:locking_gap}, \(\mathcal I_D\), supply the scalar formation interface.
...
After the scalarization map \(\mathcal C_D\), the spectral weights \(\mu_{nN}\), and, when the locking gap exists, the hard map \(\mathcal I_D:u_N\mapsto\psi_N^{\rm lock}\), the scalar two-center spectral operator and the quantities \((g_N^{\rm chiral},\Gamma_N,B_N,P_N)\) define an effective model for how these three protected modes form and couple to the visible sector.
